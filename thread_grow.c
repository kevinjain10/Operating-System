...
struct thread_mutexlock m; //lock for protecting printf
...
void do_sbrk(void *arg){
    ...
    */
	...
	thread_mutex_lock(&m);
	printf(1, "[thread : %s (sbrk)] Old process size is %d before doing sbrk(%d)\n", name, (uint)sbrk(0), a);
	thread_mutex_unlock(&m);

	char *mem = sbrk(a);

	thread_mutex_lock(&m);
	printf(1, "[thread : %s (sbrk)] New process size is %d and returned address is %d\n", name, (uint)sbrk(0), (uint)mem);
	thread_mutex_unlock(&m);
...
void do_malloc(void *arg){
    ...
    */	
	...
	thread_mutex_lock(&m);
	printf(1, "[thread: %s (malloc)] Old process size is %d before doing malloc(%d)\n",name, (uint)sbrk(0), a);
	thread_mutex_unlock(&m);

	void *mem = malloc(a);

	thread_mutex_lock(&m);
	printf(1, "[thread: %s (malloc)] New process size is %d and returned address is %d\n",name, (uint)sbrk(0), (uint)mem);
	thread_mutex_unlock(&m);
...
int main(int argc, char *argv[]) {
	  ...
	  for(i = 0; i < THREAD_NUM; i++){
	    ...
	    if(i%2 == 0)
		 t[i] = thread_create(do_sbrk, (void*)&b[i], (void*)s[i]);
	    else
		  t[i] = thread_create(do_malloc, (void*)&b[i], (void*)s[i]);
